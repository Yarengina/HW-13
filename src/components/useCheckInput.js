...
const useCheckInput = (initialValue) => {
    ...
    const onBlur = ({target: { name, value }}) => {
        switch (name) {
            case 'password':
               if(value.length && value.length < passwordLength) setIsPassError('At least 6 characters')
                break
            case 'email':
               if(value.length && !value.match(validEmail)) setIsEmailError('Email is not valid')
                break
            default: break
        }
    }
